"""Read <dir> and create dictionary/corpus """
...
if __name__ == "__main__":
    if len(argv) != 6 :
        print("Usage: {} <fulltexts> <abstracts> <dir> <#topics> <seed>\n".format(argv[0]))
        exit(1)
    ...
    ft_dir = "/home/ad/home/y/yzan/Desktop/Cleanskin/results/cs_lbsec/cs_ft"
    dictionary = Dictionary(ReadTxtFiles(ft_dir))
    print('Number of unique tokens: %d' % len(dictionary))
    corpus = [dictionary.doc2bow(doc) for doc in docs]
    ab_corpus = 0 #[dictionary.doc2bow(doc) for doc in ab_docs]
