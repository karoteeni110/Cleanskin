...
def show_examplefile(rootdir, tagname):
    ...
    for fn in listdir(rootdir):
        fpath = join(rootdir, fn)
        if fn[-3:] == 'xml':
            nodes = get_rank1(fpath)
        if set(tagname) <= nodes:
...
def check_children(parent_tag, fpath):
    ...
    try:
        ...
        root = tree.getroot()
        ignore_ns(root)
        child_taglst = [child.tag for parent in root.iter(parent_tag) 
                                    for child in list(parent)] # get rid of namespace
        allnodes.extend(child_taglst)
    except ET.ParseError:
        print('ParseError at',fpath)
    return frozenset(allnodes)
